    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="p-4 flex items-center justify-between hover:bg-gray-50">
            <div class="flex items-center space-x-4">
                ...
                <span class="text-sm {{ $subtask->is_done ? 'line-through text-gray-500' : 'text-gray-900' }}">
                    {{ $subtask->title }}
                </span>
                @if($subtask->is_done)
            ...
            <div>
                <form action="{{ route('subtasks.destroy', $subtask->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus subtask ini?')" class="inline">
                    @csrf
                    @method('DELETE')
